...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Check if its preconditions satisfied by the previous level
            if action_node.prenodes.issubset(previous_s_level):
                # Add to the list of action nodes
                action_nodes.append(action_node)

                # Connect to previous level's relevant state nodes
                for state_node in previous_s_level:
                    if state_node in action_node.prenodes:
    ...
    def add_literal_level(self, level):
        ...
        current_s_nodes = set() #Need unique values, so we using set
        previous_a_level = self.a_levels[level - 1]

        for action_node in previous_a_level:
            #  Add the effect nodes of the actions from previous level to current level's state nodes
            #  and connect each added state node to his parent action node
            for state_node in action_node.effnodes:
                current_s_nodes.add(state_node)
                action_node.children.add(state_node)
                state_node.parents.add(action_node)

        self.s_levels.append(current_s_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if one action negates an effect of the other
        for node_a1_effect in node_a1.action.effect_add:
            if node_a1_effect in node_a2.action.effect_rem:
                return True

        for node_a2_effect in node_a2.action.effect_add:
            if node_a2_effect in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if one of the effects of one action is the negation of a precondition of the other
        for node_a1_effect in node_a1.action.effect_add:
            if node_a1_effect in node_a2.action.precond_neg:
                return True
        for node_a2_effect in node_a2.action.effect_add:
            if node_a2_effect in node_a1.action.precond_neg:
                return True
        for node_a1_effect in node_a1.action.effect_rem:
            if node_a1_effect in node_a2.action.precond_pos:
                return True
        for node_a2_effect in node_a2.action.effect_rem:
            if node_a2_effect in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if one of the preconditions of one action is mutually exclusive with a
        # precondition of the other

        # Note - The parents of the node are the preconditions
        for node_a1_precond in node_a1.parents:
            for node_a2_precond in node_a2.parents:
                if node_a1_precond.is_mutex(node_a2_precond):
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for i, s_level in enumerate(self.s_levels):
                level_literals = set()
                for n in s_level:
                    literal = expr(n.symbol)
                    if not n.is_pos:
                        literal = expr('~{}'.format(n.symbol))
                    level_literals.add(literal)

                if goal in level_literals:
                    level_sum += i
                    break
